...
export const calculateInitiationDose = (
  ...
  const dosingTable: DosingTable = isSensitive
    ? {
      1: "2.5 mg",
  ...
  for (const rangeKey in dayDosing) {
    const [low, high] = rangeKey.includes("-") ? rangeKey.split("-").map(parseFloat) : [parseFloat(rangeKey.slice(1)), null];
    if (rangeKey.startsWith("<") && inr < low) {
      return `Day ${day}, INR ${rangeKey}: Recommended dose: ${dayDosing[rangeKey]}.`;
    }
    if (rangeKey.startsWith(">") && inr > low) {
      return `Day ${day}, INR ${rangeKey}: Recommended dose: ${dayDosing[rangeKey]}.`;
    }
    if (high !== null && inr >= low && inr <= high) {
      return `Day ${day}, INR ${rangeKey}: Recommended dose: ${dayDosing[rangeKey]}.`;
    }
  }
...
export const calculateWarfarinDose = (inr: number, currentDose: number): string => {
  if (inr < 1.5) {
    const newDose = currentDose * 1.2;
    return `INR < 1.5: Increase dose by 20%. New dose: ${newDose.toFixed(2)} mg daily.`;
  } else if (inr >= 1.5 && inr <= 1.9) {
    const newDose = currentDose * 1.1;
